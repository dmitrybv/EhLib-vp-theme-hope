<template><div><h1 id="set-fonts-and-coloring-grid-cells-using-events" tabindex="-1"><a class="header-anchor" href="#set-fonts-and-coloring-grid-cells-using-events" aria-hidden="true">#</a> Set fonts and coloring grid cells using events</h1>
<p>To control the rendering content of cells in the grid data partition, there are two types of events.</p>
<p>Type	1	event to replace the rendering parameters. It's events like <code v-pre>TDBVertGridEh.OnGetCellParams</code> and <code v-pre>TFieldRowEh.OnGetCellParams</code>. In these events, it is necessary to replace the parameters such as font, font, text alignment, the number of pictures <code v-pre>ImageList</code>, etc., but drawing itself carries the internal code of the grid.</p>
<p>Type	2 	events in which event handler executes rendering the cell contents.</p>
<p>Here is a list of events that you can use to customize the font attributes and color of the cell before the data is printed. At the time of calling all of the following events DataSet will already be on the record, the contents of which it is necessary to render, so you can access the field values using the methods like TDataSet.FieldByName(..).Value. Do not change the properties of the grid or columns inside those events. This may cause a program loops.</p>
<p><code v-pre>TDBVertGridEh.OnGetCellParams</code></p>
...
<p><code v-pre>TFieldRowEh.OnGetCellParams</code></p>
...
<p><code v-pre>TDBVertGridEh.OnAdvDrawDataCell</code> and <code v-pre>TFieldRowEh.OnAdvDrawDataCell</code></p>

<template><div><h1 id="show-hint-winow-for-planner-items" tabindex="-1"><a class="header-anchor" href="#show-hint-winow-for-planner-items" aria-hidden="true">#</a> Show Hint Winow for planner Items</h1>
<p>There are event in the <code v-pre>PlannerView</code> to set data for hint window (Hints) that appear when mouse you hover over the event item area.</p>
<div class="language-pascal" data-ext="pascal"><pre v-pre class="language-pascal"><code>OnSpanItemHintShow<span class="token punctuation">:</span> TPlannerViewSpanItemHintShowEventEh
</code></pre></div><dd>
<p>Write the event handler to change text, font or background color.
The hint window is appeared when a mouse moves and hold over the event area (<code v-pre>TPlannerDataItemEh</code>).</p>
<p>In the below example the <code v-pre>OnSpanItemHintShow</code> handler adds a new line with text 'NewLine' and sets the hint font in bold style.</p>
</dd>
<div class="language-pascal" data-ext="pascal"><pre v-pre class="language-pascal"><code><span class="token keyword">procedure</span> TfrFrameOne<span class="token punctuation">.</span>PlannerDayViewEh1SpanItemHintShow<span class="token punctuation">(</span>
...
</code></pre></div><p>On the screen the hint will appear as follows:</p>
<p><img src="@source/images/clip0071.png" alt="" loading="lazy"></p>
</div></template>

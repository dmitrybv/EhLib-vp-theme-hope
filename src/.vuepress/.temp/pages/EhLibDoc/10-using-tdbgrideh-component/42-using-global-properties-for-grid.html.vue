<template><div><h1 id="using-global-properties-for-tdbgrideh" tabindex="-1"><a class="header-anchor" href="#using-global-properties-for-tdbgrideh" aria-hidden="true">#</a> Using global properties for TDBGridEh</h1>
<p>The Library has a set of global properties and event, which influence all grids that are created in the project. You can adjust this properties or event, for instance, in the main Form of your project, and all grids of the project will change its behaviors according to these properties. To control the global behavior of grids there are global objects such as the <code v-pre>TDBGridEhCenter</code> and <code v-pre>TDBGridEhStyle</code> type. The First one is used to control the behavior of the grids, the second one is to control the style of grids showing.</p>
<h3 id="general-information-about-tdbgridehcenter-class" tabindex="-1"><a class="header-anchor" href="#general-information-about-tdbgridehcenter-class" aria-hidden="true">#</a> General information about <code v-pre>TDBGridEhCenter</code> class:</h3>
<p>Use global function <code v-pre>DBGridEhCenter</code> to access the elements of the <code v-pre>TDBGridEhCenter</code> object:</p>
<div class="language-pascal" data-ext="pascal"><pre v-pre class="language-pascal"><code><span class="token keyword">function</span> DBGridEhCenter<span class="token punctuation">:</span> TDBGridEhCenter<span class="token punctuation">;</span>
</code></pre></div><p>It returns the global object of the type <code v-pre>TDBGridEhCenter</code>, which controls the behavior of all grids. You may inherit <code v-pre>TDBGridEhCenter</code> class, rewrite virtual methods and register your object of managing using <code v-pre>SetDBGridEhCenter</code> function:</p>
<div class="language-pascal" data-ext="pascal"><pre v-pre class="language-pascal"><code><span class="token keyword">function</span> SetDBGridEhCenter<span class="token punctuation">(</span>NewGridCenter<span class="token punctuation">:</span> TDBGridEhCenter<span class="token punctuation">)</span><span class="token punctuation">:</span> TDBGridEhCenter<span class="token punctuation">;</span>
</code></pre></div><p>It set new object of management and returns the old object. The Old object, in most cases, is required to destroy.</p>
<p>Properties and events of the <code v-pre>TDBGridEhCenter</code> class:</p>
<p><strong>Properties:</strong></p>
<dd>
<p><code v-pre>property FilterEditCloseUpApplyFilter: Boolean;</code></p>
<dl><dd>
<p>Determines whether the <code v-pre>SubTitleFilter</code> will be applied right after choosing value from dropped down list or always need to press Enter key to apply filter.</p>
</dd></dl>
<p><code v-pre>property IndicatorTitleMenus: TDBGridEhStyleIndicatorTitleMenusEh;</code></p>
  <dl><dd>
<p>Specifies the list of elements, which will be visible in IndicatorTitle of dropped down menu when <code v-pre>TDBGridEh.IndicatorTitle.UseGlobalMenu = True</code> and <code v-pre>TDBGridEh.IndicatorTitle.TitleButton = True</code>.</p>
<p><code v-pre>TDBGridEhStyleIndicatorTitleMenusEh</code> is a set which consists of following values
itmVisibleColumnsEh	- Display the menu item with list of columns, cliquing on which possible to show or hide column.
<dl><dd><code v-pre>itmCut</code>		- Display Cut menu item</dd></dl>
<dl><dd><code v-pre>itmCopy</code>		- Display Copy menu item</dd></dl>
<dl><dd><code v-pre>itmPaste</code>		- Display Paste menu item</dd></dl>
<dl><dd><code v-pre>itmDelete</code>		- Display Delete menu item</dd></dl>
<dl><dd><code v-pre>itmSelectAll</code>	- Display SelectAll menu item</dd></dl></p>
  </dd></dl>
</dd>
<p><strong>Events:</strong></p>
<dd>
<div class="language-pascal" data-ext="pascal"><pre v-pre class="language-pascal"><code>OnApplyFilter<span class="token punctuation">:</span> TNotifyEvent<span class="token punctuation">;</span>
</code></pre></div><dl><dd>
<p>This event is called when grid apply <code v-pre>SubTitleFilter</code>. <code v-pre>Sender</code> parameter is a grid that applies filter. In the handler of event you can call default handler - <code v-pre>DBGridEhCenter.DefaultApplyFilter</code>.</p>
</dd></dl>
<div class="language-pascal" data-ext="pascal"><pre v-pre class="language-pascal"><code>OnApplySorting<span class="token punctuation">:</span> TNotifyEvent<span class="token punctuation">;</span>
</code></pre></div><dl><dd>
<p>This event is called when grid apply sorting. <code v-pre>Sender</code> parameter is a grid that applies sorting. In the event handler you can call default handler - <code v-pre>DBGridEhCenter.DefaultApplySorting</code>.</p>
</dd></dl>
<div class="language-pascal" data-ext="pascal"><pre v-pre class="language-pascal"><code>OnBuildIndicatorTitleMenu<span class="token punctuation">:</span> TDBGridEhBuildIndicatorTitleMenu<span class="token punctuation">;</span>
</code></pre></div><dl><dd>
<p>This event occurs when grid forms the dropped down menu that appears when mouse is clicked on the upper left cell of grid. Event occurs if <code v-pre>TDBGridEh.IndicatorTitle.TitleButton</code> = <code v-pre>True</code>. Sender parameter is a grid in which <code v-pre>IndicatorTitle</code> button was pressed. <code v-pre>PopupMenu</code> parameter is a menu that will is displayed under cell. You can call default handler that build menu items by default:
<code v-pre>DBGridEhCenter.DefaultBuildIndicatorTitleMenu(Grid, PopupMenu);</code>
and then perform additional code, for instance, to add one more menu elements.</p>
<p>Below is an example that demonstrate how to add new menu item
in list of IndicatorTitle menus:</p>
<div class="language-pascal line-numbers-mode" data-ext="pascal"><pre v-pre class="language-pascal"><code>
...
<div class="language-pascal" data-ext="pascal"><pre v-pre class="language-pascal"><code><span class="token keyword">property</span> OnCellMouseClick<span class="token punctuation">:</span> TGridEhCellMouseEvent<span class="token punctuation">;</span>
...
<br>
<div class="language-pascal" data-ext="pascal"><pre v-pre class="language-pascal"><code><span class="token keyword">property</span> OnIndicatorTitleMouseDown<span class="token punctuation">:</span> TGridEhCellMouseEvent<span class="token punctuation">;</span>
   <span class="token keyword">type</span>  TGridEhCellMouseEvent <span class="token operator">=</span> <span class="token keyword">procedure</span> <span class="token punctuation">(</span>Grid<span class="token punctuation">:</span> TCustomGridEh<span class="token punctuation">;</span> Cell<span class="token punctuation">:</span> TGridCoord<span class="token punctuation">;</span>
    Button<span class="token punctuation">:</span> TMouseButton<span class="token punctuation">;</span> Shift<span class="token punctuation">:</span> TShiftState<span class="token punctuation">;</span> X<span class="token punctuation">,</span> Y<span class="token punctuation">:</span> Integer<span class="token punctuation">)</span> <span class="token keyword">of</span> <span class="token keyword">object</span><span class="token punctuation">;</span>
</code></pre></div><dl><dd>
...
<br>
<div class="language-pascal" data-ext="pascal"><pre v-pre class="language-pascal"><code><span class="token keyword">property</span> OnLocateText<span class="token punctuation">:</span> TLocateTextEventEh<span class="token punctuation">;</span>
...
<h3 id="the-general-information-about-tdbgridehstyle-class" tabindex="-1"><a class="header-anchor" href="#the-general-information-about-tdbgridehstyle-class" aria-hidden="true">#</a> The General information about <code v-pre>TDBGridEhStyle</code> class</h3>
<p>To access elements the global <code v-pre>DBGridEhStyle</code> object use function:</p>
<p><code v-pre>function DBGridEhStyle: TDBGridEhStyle;</code></p>
<dl><dd>
...
<p><code v-pre>function SetDBGridEhStyle(NewGridStyle: TDBGridEhStyle): TDBGridEhStyle;</code></p>
<dl><dd>
...
<br>
<p>Properties of the <code v-pre>TDBGridEhStyle</code> class:</p>
<p><code v-pre>property LuminateSelection: Boolean default True;</code></p>
<dl><dd>
<p>This property determines whether need to illuminate the selected area in grid. If <code v-pre>LuminateSelection = False</code> then the selected area will be painted by <code v-pre>clHighlight</code> color. If <code v-pre>LuminateSelection = True</code> then selected area will be painted by method as if you see on the area through the transparent glass of the <code v-pre>clHighlight</code> color.</p>
</dd></dl>
<p><code v-pre>property IsDrawFocusRect: Boolean default True;</code></p>
<dl><dd>
